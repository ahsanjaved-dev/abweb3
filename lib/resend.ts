      ...
      <div class="content">
            ...
            <div class="service-item">
              <div class="service-header">
                <div>
                  ...
                  ${service.duration ? `<div class="service-duration">⏱️ Duration: ${service.duration}</div>` : ""}
                </div>
        ...
        <div class="section">
          ...
          <div style="color: #64748b; font-style: italic;">
            ${booking.photos.length} photo${booking.photos.length !== 1 ? "s" : ""} uploaded by customer
          </div>
        </div>
        `
            : ""
...
export const sendCustomerConfirmation = async (
  ...
  try {
    ...
    const customerEmailHTML = `
      ...
      <head>
        ...
        <style>
          ...
          .header { background: linear-gradient(135deg, #3b82f6, #6366f1); color: white; padding: 30px; border-radius: 12px; text-align: center; margin-bottom: 30px; }
          .content { background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); }
      ...
      <body>
        <div class="header">
          ...
          <p>Thank you for choosing Hoffman Cleaning</p>
        </div>
        <div class="content">
          <p>Dear ${contact.firstName},</p>
          <p>We've received your booking request and will contact you soon to confirm your appointment.</p>
          
          <div class="booking-details">
            <h3>Your Booking Details:</h3>
            <p><strong>Booking ID:</strong> ${bookingData.bookingId}</p>
            <p><strong>Date:</strong> ${formatDate(bookingData.arrivalWindow.date)}</p>
            <p><strong>Time:</strong> ${bookingData.arrivalWindow.time}</p>
            <p><strong>Total:</strong> ${formatCurrency(bookingData.total)}</p>
          </div>
          
          <p>We'll be in touch within 24 hours to confirm your appointment details.</p>
          <p>If you have any questions, please don't hesitate to contact us.</p>
          
          <p>Best regards,<br>Hoffman Cleaning Team</p>
        </div>
      </body>
      </html>
    `
    ...
    const { data, error } = await resend.emails.send({
      from: process.env.FROM_EMAIL || "bookings@hoffmancleaning.com",
      to: [contact.email],
      subject: "✅ Booking Confirmation - Hoffman Cleaning",
